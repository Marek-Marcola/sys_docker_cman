...
if [ $HELP -eq 1 ]; then
  ...
  echo "  \$V    - image version"
  echo "  \$I    - image name"
  echo "  \$WDIR - work directory"
  echo "  \$OPTS - docker options (array)"
  echo "  \$INIT - init commands  (array)"
...
if [ $INSTALL_ANPB -eq 1 ]; then
  ...
  if [ $EVAL -eq 0 ]; then
    set -ex
    anpb cman_install.yml -e h=$INSTALL_ANPB_HP --check --diff
    { set +ex; } 2>/dev/null
  else
    set -ex
    anpb cman_install.yml -e h=$INSTALL_ANPB_HP
    { set +ex; } 2>/dev/null
  fi
